...
const VERSION = "v1.2.9-20260616_LogicFixed";

const Storage = {
    save(r, p) { localStorage.setItem('mj_room', r); localStorage.setItem('mj_player', p); },
    get() { 
        const u = new URLSearchParams(window.location.search);
        return { room: u.get('room') || localStorage.getItem('mj_room'), player: u.get('player') || localStorage.getItem('mj_player') };
    },
    clear() { localStorage.removeItem('mj_room'); localStorage.removeItem('mj_player'); }
};

const safeRender = (h) => { const a = document.getElementById('app'); if(a) a.innerHTML = h; };
...
const App = {
    ...
    listenToRoom(myId) {
        db.ref(`rooms/${this.data.roomId}`).on('value', snap => {
            const data = snap.val();
            if (!data) { 
                safeRender(`<div class="card"><p>房间已失效</p><button class="btn" onclick="App.forceExit()">返回首页</button></div>`);
                return; 
            }
            ...
            } else { this.renderIdentitySelect(); }
        });
    },

    forceExit() { Storage.clear(); window.location.href = window.location.pathname; },

    // --- 核心校验：防止空分跳转 ---
    validateAndConfirmBasic() {
        const amount = this.data.game.basic?.inputs?.[this.data.myId];
        if (!amount || amount <= 0) {
            alert("请输入有效的支付分数（需大于0）");
            return;
        }
        this.updateRef(`game/basic/confirmed/${this.data.myId}`, true);
    },
    ...
    checkAutoTransition() {
        ...
        if (game.phase === 'roleSelect') {
            ...
            const b = game.basic || {};
            // 增加逻辑校验：必须 confirmed 且金额有效
            const done = game.type === 'jiepao' ? (b.confirmed?.[game.roles.loser] && (b.inputs?.[game.roles.loser] > 0)) : 
                         game.roles.losers?.every(id => b.confirmed?.[id] && (b.inputs?.[id] > 0));
    ...
    async finalizeRound() {
        ...
        if (game.type === 'jiepao') {
            ...
            game.roles.losers.forEach(id => { const val = game.basic.inputs[id] || 0; deltas[id] -= val; deltas[game.roles.winner] += val; });
        }
        ...
        Object.entries(activeBirds).forEach(([bid, b]) => {
            ...
            if (game.type === 'jiepao') { deltas[game.roles.loser] -= bVal * birdGold; deltas[game.roles.winner] += a * birdGold; deltas[bid] += (bVal - a) * birdGold; }
            else { game.roles.losers.forEach(lid => { deltas[lid] -= a * birdGold; deltas[bid] += a * birdGold; }); const unhit = N - a; deltas[bid] -= unhit * birdGold; deltas[game.roles.winner] += unhit * birdGold; }
        });
        const updates = {};
    ...
    renderGame() {
        ...
        if (game.phase === 'roleSelect' && me.role === 'player') {
            ...
            [{k:'jiepao', t:'🀄 接炮'}, {k:'fangpao', t:'💥 放炮'}, {k:'zimo', t:'⚡ 自摸'}, {k:'tazimo', t:'💸 输家'}].forEach(r => {
                html += `<button class="btn ${game.roles?.[myId]===r.key?'btn-primary':''}" onclick="App.selectRole('${r.k}')">${r.t}</button>`;
            });
        } else if (game.phase === 'basicInput') {
            ...
            if (isWinner) {
                ...
                ls.forEach(id=>s += `${players[id].name}: ${game.basic.inputs?.[id]||0} ${game.basic.confirmed?.[id]?'✓':'等待'} `);
                html += `<div class="history-item">${s}</div><button class="btn btn-primary" onclick="App.updateRef('game/basic/winnerConfirmed', true)">复核无误并进入抓鸟</button>`;
            } else if (game.roles.loser === myId || game.roles.losers?.includes(myId)) {
                html += `<div class="phase-indicator">需支付给 ${players[game.roles.winner].name}</div>
                         <input type="number" placeholder="输入分值" onchange="App.updateBasicInput(this.value)">
                         <button class="btn btn-primary" onclick="App.validateAndConfirmBasic()">确认支付</button>`;
            ...
            if (me.role === 'bird' || (bPs.length === 0 && myId === game.roles.winner)) {
                ...
                const winnerName = players[game.roles.winner]?.name;
                const loserName = game.type==='jiepao' ? players[game.roles.loser]?.name : "三位输家";
                html += `<div class="phase-indicator">抓鸟录入 (N=${me.birdTotal||8})</div>
                    <div style="font-size:12px; color:#aaa; margin-bottom:4px;">中 ${winnerName} (胡牌方) 个数:</div>
                    <input type="number" id="birdA" value="${b.a}" onchange="App.syncBirdInput()">
                    ${game.type==='jiepao' ? `<div style="font-size:12px; color:#aaa; margin:8px 0 4px;">中 ${loserName} (放炮方) 个数:</div><input type="number" id="birdB" value="${b.b}" onchange="App.syncBirdInput()">` : ''}
                ...
                html += `<div class="history-item">${s}</div><button class="btn btn-primary" onclick="App.updateRef('game/winnerConfirmedFinal', true)">确认无误，最终结算</button>`;
            }
        }
    ...
    updateRef(p, v) { db.ref(`rooms/${this.data.roomId}/${p}`).set(v); },
    async endGame() { if(confirm('确定解散？')) { await db.ref(`rooms/${this.data.roomId}`).remove(); this.forceExit(); } },
    renderJoin() { safeRender(`<div class="card"><div class="title">🀄 麻将计分工具</div><button class="btn btn-primary" onclick="App.createRoom()">创建新房间</button><input type="text" id="roomInput" placeholder="输入房间号" style="margin-top:20px"><button class="btn" onclick="App.joinRoom()">进入房间</button><div class="version-tag">${VERSION}</div></div>`); },
    async createRoom() {
        const ns = []; for (let i=0;i<4;i++){ const n=prompt(`选手${i+1}姓名：`,`选手${i+1}`); if(!n)return; ns.push(n.trim()); }
        const g = parseInt(prompt('抓鸟金：','50'))||50; const id = Math.random().toString(36).substring(2,6).toUpperCase();
        const ps = {}; ns.forEach((n,i)=>{ ps[`p${i}`]={name:n,score:0,role:'player'}; });
        await db.ref(`rooms/${id}`).set({ players:ps, settings:{birdGold:g}, game:{round:0,phase:'roleSelect',history:[]} });
        window.location.href=`?room=${id}`;
    },
    joinRoom() { const id = document.getElementById('roomInput').value.trim().toUpperCase(); if(id) window.location.href=`?room=${id}`; },
    renderIdentitySelect() {
        let h = `<div class="card"><div class="title">确认身份</div>`;
        Object.entries(this.data.players).forEach(([id,p])=>{ h+=`<button class="btn" onclick="App.selectIdentity('${id}')">${p.name}</button>`; });
        h+=`<hr><button class="btn btn-primary" onclick="App.joinAsNewBird()">新鸟民加入</button></div>`; safeRender(h);
    },
    selectIdentity(id) { Storage.save(this.data.roomId,id); window.location.href=`?room=${this.data.roomId}&player=${id}`; },
    async joinAsNewBird() {
        const n = prompt('鸟民姓名：'); if(!n)return; const t = parseInt(prompt('总鸟数(4/8)：','8'))||8;
        const bid = 'bird_'+Date.now(); await db.ref(`rooms/${this.data.roomId}/players/${bid}`).set({name:n.trim(),score:0,role:'bird',birdTotal:t});
        this.selectIdentity(bid);
    }
};
